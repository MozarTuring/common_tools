...
local function getPath(mode)
	...
	if SRC_TITLE == "net.kovidgoyal.kitty" then
...
local function tgtAction(path, tgtTitle, shell)
	...
	if tgtTitle == "notebook" then
		hs.eventtap.keyStroke({}, "b")
		usleep(500) -- Wait for the command to be processed
		hs.eventtap.keyStroke({}, "return")
	elseif tgtTitle == "com.apple.Terminal" or tgtTitle == "com.googlecode.iterm2" then
		...
		if shell == "0" then
			...
			hs.eventtap.keyStroke({}, "return")
			usleep(300)
			hs.eventtap.keyStrokes("cd ~/project_remote && nvim .")
			usleep(1000)
			hs.eventtap.keyStroke({}, "return")
			usleep(1000)
		...
		elseif shell == "1" then
			...
			-- hs.eventtap.keyStroke({}, "return")
			-- usleep(500)
			-- local tmp_content = hs.pasteboard.getContents()
			-- hs.eventtap.keyStrokes(tmp_content)
			hs.eventtap.keyStroke({ "cmd" }, "v")
			usleep(1000)
			hs.eventtap.keyStroke({}, "return")
		elseif shell == "2" then
			hs.eventtap.keyStrokes("cd ~/project_remote_jwm")
			usleep(1000)
			hs.eventtap.keyStroke({}, "return")
			usleep(1000)
			hs.eventtap.keyStrokes("rm " .. path)
			usleep(1000)
			hs.eventtap.keyStroke({}, "return")
			usleep(1000)
			hs.eventtap.keyStrokes("vim " .. path)
			usleep(1000)
			hs.eventtap.keyStroke({}, "return")
			usleep(1000)
			hs.eventtap.keyStroke({ "cmd" }, "v")
			local clipLen = string.len(hs.pasteboard.getContents() or "")
			local sleepMs = math.max(1500, math.min(clipLen * 0.2, 4000))
			usleep(sleepMs)
...
local function share1(tgtTitle, shell)
	...
	if title:lower():find(SRC_TITLE:lower(), 1, true) then
		...
		if title:lower() == "com.googlecode.iterm2" or title:lower() == "net.kovidgoyal.kitty" then
			if shell == "0" or shell == "2" then
				if not path then
					hs.alert.show("No path on clipboard")
					return
				end
		...
		if not tgtAction(path, tgtTitle, shell) then
