...
// cross-file .md links: open in Neovim and redirect browser
use(async (req, res, next) => {
  if (/\.md$/i.test(req.asPath)) {
    const plugin = req.plugin
    const fileDir = await plugin.nvim.call('expand', `#${req.bufnr}:p:h`)
    const mdPath = path.resolve(fileDir, decodeURIComponent(req.asPath.slice(1)))
    if (fs.existsSync(mdPath)) {
      await plugin.nvim.command('edit ' + mdPath.replace(/ /g, '\\ '))
      const newBuf = await plugin.nvim.call('bufnr', '%')
      res.writeHead(302, { Location: `/page/${newBuf}` })
      res.end()
      return
    }
  }
